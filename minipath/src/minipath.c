...
static WCHAR* const  g_tchAvailableLanguages = L"af-ZA be-BY de-DE el-GR en-GB en-US es-ES fi-FI fr-FR hi-IN hu-HU id-ID it-IT ja-JP ko-KR nl-NL pl-PL pt-BR pt-PT ru-RU sk-SK sv-SE tr-TR vi-VN zh-CN zh-TW";
#endif
...
CASE_WM_CTLCOLOR_SET:
    ...
    case WM_SYSCOLORCHANGE: {
        LRESULT const lret = DefWindowProc(hwnd,umsg,wParam,lParam);
    ...
    case WM_NCXBUTTONDOWN:
        ...
        case XBUTTON1:
            return SendWMCommand(hwnd, IDT_HISTORY_BACK);
        case XBUTTON2:
            return SendWMCommand(hwnd, IDT_HISTORY_FORWARD);
        default:
            break;
        }
    ...
    case WM_NCRBUTTONUP:
    case WM_NCMBUTTONUP: {
        HWND const hwndTT = (HWND)SendMessage(hwndToolbar,TB_GETTOOLTIPS,0,0);
        if (wParam != HTCAPTION) {
            ...
            case L'P':
                if (*CharUpper(lp1+1) == L'D' || *CharUpper(lp1+1) == L'S') {
                    ...
                    if (itok == 4) { // scan successful
                        flagPosParam = 1;
                        if (Settings.wi.cx < 1) {
                            Settings.wi.cx = CW_USEDEFAULT;
                        }
                        if (Settings.wi.cy < 1) {
                            Settings.wi.cy = CW_USEDEFAULT;
                        }
                    }
                }
                break;
