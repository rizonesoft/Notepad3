...
#define NEW_EXPR(type, fmt, ...) new_expr((type), (fmt), (const te_expr*[]){ __VA_ARGS__ })
...
static te_expr *new_expr(const int type, const int fmt, const te_expr *parameters[]) {
...
static double parse_number(const char *pstrg, char **pendptr, int* type) {
    if (pstrg[0] == '0' && (strlen(pstrg) > 2)) {
        ...
        case 'X':
            *type = TOK_NUM_HEX;
            return (double)strtoll(&pstrg[2], pendptr, 16);
        default:
            break;
        }
    }
    //~*type = TOK_NUM_STD; don't change type
...
void next_token(state *s) {
    ...
    do {
        ...
        if ((s->next[0] >= '0' && s->next[0] <= '9') || s->next[0] == '.') {
            s->value = parse_number(s->next, (char**)&s->next, &s->type);
        } else {
...
static te_expr *base(state *s) {
    ...
    switch (TYPE_MASK(s->type)) {
        ...
        case TOK_NUM_BIN:
        case TOK_NUM_OCT:
            ret = new_expr(TE_CONSTANT, s->type, 0);
            if (ret) {
                ret->value = s->value;
                next_token(s);
                if (s->type == TOK_INFIX && (
                    ...
                    te_expr* ret_new = NEW_EXPR(TE_FUNCTION1, s->type, ret);
                    if (ret_new) {
                      ret = ret_new;
        ...
        case TOK_VARIABLE:
            ret = new_expr(TE_VARIABLE, TOK_NUM_STD, 0);
            if (ret) {
                ret->bound = s->bound;
                next_token(s);
            }
        ...
        case TE_FUNCTION0:
        case TE_CLOSURE0:
            ret = new_expr(s->type, TOK_NUM_STD, 0);
            if (ret) {
                ret->function = s->function;
...
static te_expr *power(state *s) {
    ...
    if (ret && (sign == 1)) {
        if (logical == 0) {
            ...
            te_expr* ret_new = NEW_EXPR(TE_FUNCTION1 | TE_FLAG_PURE, TOK_NUM_STD, ret);
            if (ret_new) {
                ret = ret_new;
...
static te_expr *factor(state *s) {
    ...
    while (s->type == TOK_INFIX && check_is_equal_function_pointer_2d(s->function, pow)) {
        ...
        if (insertion) {
            ...
            te_expr* insert = NEW_EXPR(TE_FUNCTION2 | TE_FLAG_PURE, TOK_NUM_STD, insertion->parameters[1], param);
            if (insert) {
                insert->function = t;
            ...
            te_expr* ret_new = NEW_EXPR(TE_FUNCTION2 | TE_FLAG_PURE, TOK_NUM_STD, ret, param);
            if (ret_new) {
                ret = ret_new;
                ret->function = t;
                insertion = ret;
