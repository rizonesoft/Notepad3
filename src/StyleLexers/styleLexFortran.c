...
KEYWORDLIST KeyWords_Fortran =
{
    ...
    "backspace bind blank block blockdata call case character class close codimension common complex concurrent "
    "contains contiguous continue criticle cycle "
    "data deallocate decimal delim default deferred dimension direct do dowhile double doubleprecision "
    "elemental else elseif elsewhere encoding end endassociate endblockdata enddo endfile endforall endfunction "
    "endif endinterface endmodule endprogram endselect endsubroutine endtype endwhere entry enum enumerator eor "
    "equivalence err error errmsg exist exit extends external file final flush fmt forall form format formatted function "
    "generic go goto id if images implicit import impure in include inout integer inquire intent interface intrinsic iomsg "
    "iolength iostat kind len lock logical memory module name named namelist nextrec nml none non_intrinsic non_overridable "
    "nopass nullify number only open opened operator optional out "
    "pad parameter pass pause pending pointer pos position precision print private procedure program protected public pure "
    "quote rank readwrite read real rec recl recursive result return rewind save select selectcase selecttype selectrank "
    "sequence sequential sign size stat status stop stream subroutine sync target then to type unformatted unit unlock use "
    ...
    "atomic_add atomic_and atomic_cas atomic_define atomic_fetch_add atomic_fetch_or atomic_fetch_xor atomic_or atomic_ref "
    "atomic_xor bessel_j0 bessel_j1 bessel_jn bessel_y0 bessel_y1 bessel_yn bge bgt bit_size ble blt btest ceiling char cmplx "
    "co_broadcast co_max co_min co_reduce co_sum command_argument_count conjg cos coshape count cpu_time cshift "
    "date_and_time dble digits dim dot_product dprod dshiftl dshiftr eoshift epsilon erf erfc erfc_scaled event_query "
    ...
    "huge hypot iachar iall iand iany ibclr ibits ibset ichar ieor image_index index int ior ipartity ishft ishftc "
    ...
    "system_clock tan tanh team_number this_image tiny trailz transfer transpose trim ubound uconbound unpack verify "
...
EDITLEXER lexFortran =
{
    ...
    &KeyWords_Fortran, {
        ...
        { {SCE_F_WORD2}, IDS_LEX_STR_63273, L"Function", L"bold; italic; fore:#8000FF", L"" },
